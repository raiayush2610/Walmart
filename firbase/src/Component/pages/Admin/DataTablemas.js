  ...
  try {
    ...
    switch (placeholder) {
      ...
      case "Seatno":
        ...
        break;
        case "orderby":
        await axios.get(`http://localhost:2610/order/slave/${text}`).then((res)=>((setOrder(res.data))))
        break;      
      default:
        console.log("no");
          break;
...
return (
    ...
    <h5> Search for paticular item</h5>
      <form className='form-doc'>
            ...
            <input type="radio" name="search" 
                    value="Seatno" onChange={handleChange} />
            <label>Seat No</label>

            <input type="radio" name="search" 
                    value="orderby" onChange={handleChange} />
            <label>Orderby</label>
 ...
 {(ord.Orderby == null) ? <td>null</td> : <td>{ord.Orderby}</td>}
